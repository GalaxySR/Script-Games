...
Settings.Position = UDim2.new(0.927272737, 0, 0.885714173, 0)
Settings.Size = UDim2.new(0, 40, 0, 40)
Settings.Image = "rbxassetid://14075012386"

-- Scripts

local function RTIIA_fake_script() -- Aimbot.Aimbot 
...
coroutine.wrap(RTIIA_fake_script)()
local function FXOS_fake_script() -- Highlight.Highlight 
	local script = Instance.new('LocalScript', Highlight)

	local highlightOn = false
	
	-- Get the button
	local button = script.Parent -- Assuming this script is inside a LocalScript which is inside the button
	
	local function highlightPlayer(player, shouldHighlight)
		local character = player.Character
		if character then
			-- Check if the player has a "Highlight" instance in their character and destroy it
			...
			-- Only add highlight if it should be highlighted
	...
	local function updatePlayers()
		-- Get the local player and their team
		local localPlayer = game.Players.LocalPlayer
		local localTeam = localPlayer.Team
	
		-- Loop through all players and highlight those not on the same team as the local player
		for _, player in pairs(game.Players:GetPlayers()) do
			-- Only highlight if the player is in the opposite team
			if player.Team and player.Team ~= localTeam then
				highlightPlayer(player, highlightOn)
			else
				highlightPlayer(player, false) -- Remove highlight if they are not the opposite team
			end
		end
	end
	
	local function loopHighlight()
		-- Continuously update players while the highlight is on
	...
	-- When a player leaves, ensure their highlight is removed
	game.Players.PlayerRemoving:Connect(function(player)
		highlightPlayer(player, false)
	end)
	
	-- When a player joins, ensure their highlight is set correctly
	game.Players.PlayerAdded:Connect(function(player)
		if highlightOn and player.Team and player.Team ~= game.Players.LocalPlayer.Team then
			highlightPlayer(player, true)
		end
	end)
	
end
coroutine.wrap(FXOS_fake_script)()
local function CCSAHTJ_fake_script() -- WalkSpeed.WalkSpeed 
...
coroutine.wrap(CCSAHTJ_fake_script)()
local function LBXDESG_fake_script() -- Next.LocalScript 
...
coroutine.wrap(LBXDESG_fake_script)()
local function NILHEID_fake_script() -- Back.LocalScript 
...
coroutine.wrap(NILHEID_fake_script)()
local function PJOZX_fake_script() -- Paintball.Open 
...
coroutine.wrap(PJOZX_fake_script)()
local function NJOYHU_fake_script() -- Paintball.Mouse 
	local script = Instance.new('LocalScript', Paintball)
	...
	gui:GetPropertyChangedSignal("Enabled"):Connect(updateMouseBehavior)
	updateMouseBehavior()
	
end
coroutine.wrap(NJOYHU_fake_script)()
